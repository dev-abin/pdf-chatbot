...
def test_chunks_keep_source_and_page_number():
    chunks = split_pages([Page(number=4, text="A" * 30)], "guide.pdf", chunk_size=20, overlap=5)
...
def test_retrieval_ranks_matching_chunk_first():
    chunks = split_pages(
        [Page(number=1, text="Python uses indentation to define code blocks."), Page(number=2, text="PDF pages can contain images.")],
        "notes.pdf",
        chunk_size=200,
        overlap=20,
    )
